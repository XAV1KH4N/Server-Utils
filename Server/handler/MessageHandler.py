...
from common.events.Events import Event, EventDesitination, EventOrigin, EventPublisher
from messages.common.MessageBuilder import MessageBuilder
...
class MessageHandler(EventPublisher):
    ...
    def handle_communication_event(self, event: Event) -> None:
        match event:
            case ClientMessageEvent():
                self.handle_raw_data(event.get_data())
            case _ :
    ...
    def handle_msg(self, msg: Serializable) -> None: 
        match msg:
            case KeyExchangeResponseMessage():
                other_y = msg.get_Y()               
                self.publish(KeyExchangeResponseEvent(other_y))
            case EncryptedMessage():
                self.publish(EncryptedMessageEvent(msg))
            case TextMessage():
                print("Encrypted Text Message", msg.get_msg())
            case _ :
                log("Unhandled client message") 

class KeyExchangeResponseEvent(Event):
